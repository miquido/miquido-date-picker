import {
  ...
  isValidaDayFormat,
  isValidMonthFormat,
  isValidYearFormat, isValidDate, replaceCharInText
} from './functions'
...
describe('functions', () => {
  ...
  // ------------- selectDate -------------
  it('selectDate should return day with selected: true', () => {
    expect(selectDate(dayObjectMock)).toHaveProperty('selected', true)
  })
  // ------------- unselectDate -------------
  it('unselectDate should return day with selected: false', () => {
    expect(unselectDate(dayObjectMock)).toHaveProperty('selected', false)
  })
  // ------------- disableDay -------------
  it('disableDay should return day with disabled: true', () => {
    expect(disableDay(dayObjectMock)).toHaveProperty('disabled', true)
  })
  // ------------- enableDay -------------
  it('enableDay should return day with disabled: false', () => {
    expect(enableDay(dayObjectMock)).toHaveProperty('disabled', false)
  })
  // ------------- getDaysInMonth -------------
  it('getDaysInMonth should return number of days in month', () => {
    expect(getDaysInMonth(11, 2018)).toEqual(31)
  })
// ------------- generateCalendar -------------
  it('generateCalendar should return array of day objects', () => {
  ...
  it('generateCalendar should return array with correct length', () => {
    expect(generateCalendar(11, 2018)).toHaveLength(31)
  })
  it('generateCalendar should mark today', () => {
    const now = new Date()
    const indexOfCurrentDay = now.getDate() - 1
...
// ------------- defaultClassNameShouldBeOverwritten -------------
  it('defaultClassNameShouldBeOverwritten check if class should be overwritten', () => {
...
// ------------- getClassFor -------------
  it('getClassFor should return className from theme if provided', () => {
    const getClassObjectWithTheme = {
      key: 'picker',
      theme: tripGateTheme,
      defaultClass: picker
    }
    const expected = tripGateTheme['picker']
    expect(getClassFor(getClassObjectWithTheme)).toEqual(expected)
  })
  it('getClassFor should return default className if there is no theme provided', () => {
    const getClassObjectNoTheme = {
      key: 'picker',
      defaultClass: picker
    }
...
// ------------- getDisplayValue -------------
  it('getDisplayValue should return display value', () => {
    expect(getDisplayValue(dayObjectsArrayMock, 0)).toEqual(1)
  })
// ------------- assembleDate -------------
  it('assembleDate should return correct date', () => {
    expect(assembleDate(1, 1, 11, 2018)).toEqual('01/12/2018')
  })
  it('assembleDate should return empty string if there is no end date', () => {
    expect(assembleDate(1, undefined, 11, 2018)).toEqual('')
  })
  it('assembleDate should return correct date if end and start are reversed', () => {
    expect(assembleDate(2, 1, 11, 2018)).toEqual('')
  })
  it('assembleDate should return single date if start and end are equal', () => {
    expect(assembleDate(1, 1, 11, 2018)).toEqual('01/12/2018')
  })
  it('assembleDate should add zero if day is below 10', () => {
    expect(assembleDate(1, 1, 11, 2018)).toEqual('01/12/2018')
  })
  it('assembleDate should add zero if month is below 10', () => {
    expect(assembleDate(1, 1, 1, 2018)).toEqual('01/02/2018')
  })
  it('assembleDate should return month number not index', () => {
    const monthIndex = 0
    expect(assembleDate(1, 1, monthIndex, 2018)).toEqual(`01/0${monthIndex + 1}/2018`)
  })
// ------------- assembleMultiSelectDate -------------
  it('assembleMultiSelectDate return correct date', () => {
    expect(assembleMultiSelectDate(new Date(2018, 11, 0), new Date(2019, 0, 1)))
      .toEqual('30/11/2018-01/01/2019')
  })
  it('assembleMultiSelectDate should add 0 if start day is lower than 10', () => {
    expect(assembleMultiSelectDate(new Date(2018, 11, 1), new Date(2019, 0, 1)))
      .toEqual('01/12/2018-01/01/2019')
  })
  it('assembleMultiSelectDate should add 0 if start month is lower than 10', () => {
    expect(assembleMultiSelectDate(new Date(2018, 0, 1), new Date(2019, 0, 1)))
      .toEqual('01/01/2018-01/01/2019')
  })
  it('assembleMultiSelectDate should add 0 if end day is lower than 10', () => {
    expect(assembleMultiSelectDate(new Date(2018, 0, 1), new Date(2019, 0, 1)))
      .toEqual('01/01/2018-01/01/2019')
  })
  it('assembleMultiSelectDate should add 0 if end month is lower than 10', () => {
    expect(assembleMultiSelectDate(new Date(2018, 0, 1), new Date(2019, 0, 1)))
      .toEqual('01/01/2018-01/01/2019')
  })
// ------------- assembleDateForStartOnly -------------
  it('assembleDateForStartOnly return correct start date', () => {
    expect(assembleDateForStartOnly(new Date(2018, 11, 0)))
      .toEqual('31/11/2018')
  })
  // ------------- assembleDateForEndOnly -------------
  it('assembleDateForEndOnly return correct end date', () => {
    expect(assembleDateForEndOnly(new Date(2018, 11, 0)))
      .toEqual('31/11/2018')
  })
  // ------------- setRawStyles -------------
  it('setRawStyles to set animation styles', () => {
    expect(setRawStyles).toBeDefined()
  })
  // ------------- checkVisibleCalendarForSelection -------------
  it('checkVisibleCalendarForSelection should return true if selection is in display range', () => {
    const selectedDate = {
  ...
  it('checkVisibleCalendarForSelection should return false if selection is out of display range', () => {
    const selectedDate = {
      start: new Date(2018, 11, 1),
      end: new Date(2018, 11, 15),
      display: new Date(2018, 11)
  ...
  // ------------- getLastDayOfAMonth -------------
  it('getLastDayOfAMonth should return index of last day of a month', () => {
    expect(getLastDayOfAMonth(2018, 11)).toEqual(31)
    expect(getLastDayOfAMonth(2018, 1)).toEqual(28)
  })
  // ------------- isNumeric -------------
  it('isNumeric should return true for valid number', () => {
  ...
  it('isNumeric should return false for invalid number', () => {
  ...
  // ------------- charWasDeleted -------------
  it('charWasDeleted should compare two strings length and return true if current is shorter', () => {
    expect(charWasDeleted('asd', 'as')).toEqual(true)
  })
  it('charWasDeleted should compare two strings length and return false if current is longer or equal', () => {
    expect(charWasDeleted('asd', 'asd')).toEqual(false)
    expect(charWasDeleted('asd', 'asdf')).toEqual(false)
  })
  // ------------- isValidaDayFormat -------------
  it('isValidaDayFormat should check format for number or string', () => {
    expect(isValidaDayFormat(23, 31)).toEqual(true)
    expect(isValidaDayFormat('23', 31)).toEqual(true)
  })
  it('isValidaDayFormat should return false for empty string', () => {
    expect(isValidaDayFormat('', 31)).toEqual(false)
  })
  it('isValidaDayFormat should return false for no number values', () => {
    expect(isValidaDayFormat('NAN', 31)).toEqual(false)
  })
  it('isValidaDayFormat should return false if value doesnt match pattern', () => {
    expect(isValidaDayFormat(1, 31)).toEqual(false)
  })
  it('isValidaDayFormat should return false if value extends days in month', () => {
    expect(isValidaDayFormat(50, 31)).toEqual(false)
  })
  // ------------- isValidMonthFormat -------------
  it('isValidMonthFormat should check format for number or string', () => {
    expect(isValidMonthFormat(10)).toEqual(true)
    expect(isValidMonthFormat('07')).toEqual(true)
  })
  it('isValidMonthFormat should return false for empty string', () => {
    expect(isValidMonthFormat('')).toEqual(false)
  })
  it('isValidMonthFormat should return false for no number values', () => {
    expect(isValidMonthFormat('NAN')).toEqual(false)
  })
  it('isValidMonthFormat should return false if value doesnt match pattern', () => {
    expect(isValidMonthFormat(1)).toEqual(false)
  })
  it('isValidMonthFormat should return false if value extends 12', () => {
    expect(isValidMonthFormat(50)).toEqual(false)
  })
  // ------------- isValidYearFormat -------------
  it('isValidYearFormat should check format for number or string', () => {
    expect(isValidYearFormat(2023)).toEqual(true)
    expect(isValidYearFormat('2099')).toEqual(true)
  })
  it('isValidYearFormat should return false for empty string', () => {
    expect(isValidYearFormat('')).toEqual(false)
  })
  it('isValidYearFormat should return false for no number values', () => {
    expect(isValidYearFormat('NAN')).toEqual(false)
  })
  it('isValidYearFormat should return false if value doesnt match pattern', () => {
    expect(isValidYearFormat(1)).toEqual(false)
  })
  // ------------- isValidDate -------------
  it('isValidDate should return true if value match pattern', () => {
    expect(isValidDate('11/11/2018')).toEqual(true)
  })
  it('isValidDate should return false if value doesnt match pattern', () => {
    expect(isValidDate('2018/11/11')).toEqual(false)
  })
  // ------------- replaceCharInText -------------
  it('replaceCharInText should replace char in text at given position', () => {
    expect(replaceCharInText(2, 'test to replace', 'a')).toEqual('tast to replace')
  })
  it('replaceCharInText should return orginal text if position is not valid', () => {
    expect(replaceCharInText(NaN, 'test to replace', 'a')).toEqual('test to replace')
  })
  it('replaceCharInText should return orginal text if position is bigger than text length', () => {
